...
for iter in range(N_ITER):
    ...
    df = pd.read_csv(f"./data-classified/pimpo/df_pimpo_{N_SAMPLE}_{MODEL_NAME}_{DATE}_{iter}.zip")
    ...
    #    vectorizer = "multi"
        ...
        raise Exception("vectorizer not found for ", MODEL_NAME)
    if MODEL_NAME:
        size = "base"
    ...
    #    size = "base"
        ...
        raise Exception("size not found for ", MODEL_NAME)
    #if MODEL_NAME:
    #    langs = "en-de-sv-fr"
    ...
    df_cl["label_pred"] = df_cl["label_pred"].astype(int)
    ...
    # groups to analyse
    meta_data_groups = ["country_iso", "language_iso", "parfam_text", "parfam_rile", "label_rile", "decade", "label_text"]  #"label_text",
    ...
    for group in meta_data_groups:
        ...
        group_label_count = df_cl.groupby(by=group).apply(lambda x: x.label.astype(int).value_counts(sort=True).to_frame())  #.stack()  #.to_frame()
        ...
        # create dic for rows for predicted label counts
...
## good summary dic with main results for each classifier
meta_metric_per_classifier_dic
...
df_corr_cl = df_corr[['meta data', 'algorithm', 'language representation', 'algorithm size',
                       'average correlation', 'average p-value',  #'training languages',
...
df_corr_meta = df_corr_cl.merge(df_meta_metrics_for_corr, on=["meta data", "algorithm", "language representation",
                                            "algorithm size"], how="right")  #"training languages"
